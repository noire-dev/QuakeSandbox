...
void UI_SPKickDupe_f( void )
{
	...
	for( i = 0; i < numPlayers; i++ ) {
		...
		Q_strncpyz(name, Info_ValueForKey( info, "n" ), MAX_PLAYER_NAME);
		for (j = i + 1; j < numPlayers; j++) {
			trap_GetConfigString( CS_PLAYERS + j, info2, MAX_INFO_STRING );
			if (!atoi( Info_ValueForKey( info2, "skill" ) ))
				continue;

			if (Q_stricmpn(name, Info_ValueForKey( info2, "n" ), MAX_PLAYER_NAME))
				continue;

            trap_Print(va("Duplicate kicked: %s\n", name));
...
static void UI_SPPostgameMenu_AgainEvent( void* ptr, int event )
{
	...
	UI_PopMenu();
	trap_Cmd_ExecuteText( EXEC_APPEND, "set activeAction uie_kickdupe\nmap_restart 0\n" );
}
