...
static void UI_SPLevelMenu_ResetDraw( void ) {
	UI_DrawString( SCREEN_WIDTH/2, 356 + PROP_HEIGHT * 0, "WARNING: This resets all of the", UI_CENTER|UI_SMALLFONT, color_yellow );
	UI_DrawString( SCREEN_WIDTH/2, 356 + PROP_HEIGHT * 1, "single player game variables.", UI_CENTER|UI_SMALLFONT, color_yellow );
	UI_DrawString( SCREEN_WIDTH/2, 356 + PROP_HEIGHT * 2, "Do this only if you want to", UI_CENTER|UI_SMALLFONT, color_yellow );
	UI_DrawString( SCREEN_WIDTH/2, 356 + PROP_HEIGHT * 3, "start over from the beginning.", UI_CENTER|UI_SMALLFONT, color_yellow );
}
...
static void UI_SPLevelMenu_AwardEvent( void* ptr, int notification ) {
	...
	UIE_AwardIcons_PlaySound(&levelMenuInfo.awards, (menucommon_s*)ptr);
}
...
//	int				fraglimit;
	...
	UIE_AwardIcons_DrawValues(&levelMenuInfo.awards);
...
static void UI_SPLevelMenu_Init( void ) {
	...
	// setup and display the award icons
	levelMenuInfo.awards.iconCentralGap = 64 + 16;
	UIE_AwardIcons_InitControls(&levelMenuInfo.menu, &levelMenuInfo.awards, UI_SPLevelMenu_AwardEvent);
...
/*
=================
AwardIcons_PositionX
=================
*/
static int AwardIcons_PositionX(awardInfo_t* a, int n)
{
	int x;
...
/*
=================
UIE_AwardIcons_Cache
=================
*/
void UIE_AwardIcons_Cache(awardInfo_t* a)
{
	int n;

	for( n = 0; n < 6; n++ ) {
		trap_R_RegisterShaderNoMip( ui_medalPicNames[n] );
		a->sounds[n] = trap_S_RegisterSound( ui_medalSounds[n], qfalse );
	}
}
...
/*
=================
UIE_AwardIcons_InitControls
=================
*/
void UIE_AwardIcons_InitControls(menuframework_s* m, awardInfo_t* a, callbackFunc callback)
{
	int n, x, y;
	int count;

	for( n = 0; n < 6; n++ ) {
		a->levels[n] = UI_GetAwardLevel( n );
	}
	a->levels[AWARD_FRAGS] = 100 * (a->levels[AWARD_FRAGS] / 100);
	...
	for( n = 0; n < 6; n++ ) {
		if( a->levels[n] ) {
			x = AwardIcons_PositionX(a, count);

			a->item[count].generic.type		= MTYPE_BITMAP;
			a->item[count].generic.name		= ui_medalPicNames[n];
			a->item[count].generic.flags		= QMF_LEFT_JUSTIFY|QMF_SILENT|QMF_MOUSEONLY;
			a->item[count].generic.x			= x;
			a->item[count].generic.y			= y;
			a->item[count].generic.id			= ID_AWARD1 + n;
			a->item[count].generic.callback		= callback;
			a->item[count].width				= 48;
			a->item[count].height				= 48;
			count++;
		}
	}

	for( n = 0; n < count; n++ ) {
		Menu_AddItem( m, &a->item[n] );
	}
}
...
=================
*/
void UIE_AwardIcons_PlaySound(awardInfo_t* a, menucommon_s* ptr)
{
	int n;

	n = ptr->id - ID_AWARD1;
	trap_S_StartLocalSound( a->sounds[n], CHAN_ANNOUNCER );
}
