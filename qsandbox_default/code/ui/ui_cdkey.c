...
static void UI_CDKeyMenu_DrawKey( void *self ) {
	...
	if( val == 1 ) {
		UI_DrawString( 320, 376, "Please enter your CD Key", UI_CENTER|UI_SMALLFONT, color_yellow );
	}
	else if ( val == 0 ) {
		UI_DrawString( 320, 376, "The CD Key appears to be valid, thank you", UI_CENTER|UI_SMALLFONT, color_white );
	}
	else {
		UI_DrawString( 320, 376, "The CD Key is not valid", UI_CENTER|UI_SMALLFONT, color_red );
	}
}
...
static void UI_CDKeyMenu_Init( void ) {
	...
	cdkeyMenuInfo.menu.native 	  = qfalse;
